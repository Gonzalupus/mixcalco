...
Como muchos de los millenials urbanos y con acceso a una computadora, mi integración al mundo digital inició con Windows, y salvo por una breve experimentación con ubuntu en mi adolescencia, nunca había usado otro sistema operativo. Una de las razones por las que he estado confinado a Windows es que linux y sus múltiples distribuciones no son muy conocidas para lxs usuarios *casuales* (por hacer una analogía con la jerga gamer) en oposición a los usuarios con mayor experiencia, interés, o habilidad en informática. Otra razón es que, en la percepción de estos mismos usuarios, la idea de utilizar la computadora a través de algo que no sea una interfaz gráfica (o *Graphical User Interface* en inglés) produce un sudor de manos y poco interés. En mi caso nunca se me había ocurrido usar Linux porque sentía que no contaba con el conocimiento y la capacidad para configurar y usar un sistema operativo que requiriera algo más que unos cuantos clicks ¿Qué cambió entonces?

```{r, out.width='20%', fig.cap="¡Alerta de Spoiler!: En la más reciente película de Spiderman, Miles Morales descubre que lo que necesita para poder dominar sus habilidades es confiar en sí mismo y simplemente hacer las cosas."}

knitr::include_graphics(here('leap.jpg'))
...
```{r, out.width='20%', fig.cap="En años pasados Elsevier, una de las editoriales científicas mñas poderosas demandó a Scie Hub por derechos de autor, obligando a su creadora a mudarse a Rusia."}

knitr::include_graphics(here(c('sci-hub.jpg','elsevier.jpg')))
...
```{r, fig.cap="Logos de las distribuciones Linux Mint, Ubuntu, y Manjaro"}

knitr::include_graphics(c('Linux_Mint.png', 'ubuntu.png', 'manjaro.png'),
                        path =  here(), auto_pdf = FALSE)
```
...
knitr::include_graphics(path = here('grub.png'))

```

* Desempeño en general: 

  + La apertura de programas, transferencias de archivos entre carpetas a medios externos, asì como la carga de varios libros en Okular es instantánea. 
...
knitr::include_graphics(here('escritorio.png'))
...
* Actualizaciones constantes: Manjaro es un sistema *rolling release* lo que significa que los paquetes y bases del sistema constantemente son actualizadas. Si bien los usuarios deciden qué y cuándo actualizar su sistema, es posible que algunas actualizaciones grandes rompan el sistema.
...
* Página oficial de Majaro.

* Instalación e información sobre Manjaro.

* Razones por las cuáles elegir Manjaro sobre otras distribuciones.
